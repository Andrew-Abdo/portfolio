...
void Moteur::avancerBoost(){
    ...
    ajusterPWM(vitesseBoost, vitesseBoost); // ceci est pour contrer la friction
    commun::delai_ms(30);
    arreter();
}
...
// pour tourner les coins ou pour retrouver une ligne dans la section COUPURES
...
void Moteur::avancerAjustementMur(){
    ...
    while(!estLigne){
        ...
        //Si la distance est plus grande que 16 cm
        if(sonar_.wavelength > DISTANCE_MUR_MAX){
            del_.allumerDEL(ROUGE);
            OCR1A = vitesseD + REDRESSEMENT_MUR; //augmenter la roue droite
            OCR1B = vitesseG - REDRESSEMENT_MUR; //diminuer la roue gauche
        }
        else if(sonar_.wavelength < DISTANCE_MUR_MIN){
            del_.allumerDEL(ROUGE);
            OCR1A = vitesseD - REDRESSEMENT_MUR; //diminuer la roue droite
            OCR1B = vitesseG + REDRESSEMENT_MUR; //augmenter la roue gauche
        }
        else{
...
void Moteur::avancerAjustementCouloir(){
    avancer();
    bool estFini = false;
    // rebondi entre les segments du couloir jusqu'au segment GH
    while(!estFini){
